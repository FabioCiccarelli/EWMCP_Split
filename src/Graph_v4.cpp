...
/********************************************************************************************************************************/
{
	...
	for(int e=0;e<edges;e++){weights_arcs[e]=0.0;}
	for(int v=0;v<nodes;v++){weights_nodes[v]=0.0;}
...
//		heads[i]=v2-1;
		...
		tails[i]=min(v1-1,v2-1);
		heads[i]=max(v1-1,v2-1);
...
/********************************************************************************************************************************/
{
	...
	sprintf(probname, "%s_dot_graph.txt",nome_file);
...
/********************************************************************************************************************************/
{
	...
	for (i = 0; i < numero_punti; ++i){
		...
		//sprintf(buf_, "node  [ id  %d  graphics  [ x %f   y %f  w 28 h 8 type \"roundrectangle\" ]  LabelGraphFFics  [ text  %s fontSize  4 ]  ]  ",i,vet_x[i]*coef_scala,vet_y[i]*coef_scala,buf_COOR);
		grafo  << buf_ << endl;
	}
		...
		for (i = 0; i < numero_punti; ++i){
			for (j = 0; j < numero_punti; ++j){
				if(matrice_Dist[i][j]>0){
					...
					sprintf(buf_, " edge   [ source  %d  target  %d  graphics  [ targetArrow \"delta\" Line  [ point  [ x %.2f  y %.2f  ]  point  [ x %.2f  y %.2f  ]  ]  ]  ]  ",i,j,vet_x[i]*coef_scala,vet_y[i]*coef_scala,vet_x[j]*coef_scala,vet_y[j]*coef_scala);
					grafo  << buf_ << endl;
				}
			}
		}
...
/********************************************************************************************************************************/
void read_node_weights(graphFF G,char * inFile)
/********************************************************************************************************************************/
{
...
/********************************************************************************************************************************/
void read_edge_weights(graphFF G,char * inFile)
/********************************************************************************************************************************/
{
...
//	for(int i=0;i<G->m;i++)
//	{
//		cout << "edge\t" << i << "\t tail \t" << G->T[i] << "\t head \t" << G->H[i] << "\t weight \t"  << G->P[i]  << endl;
//	}
//	cout << endl;
